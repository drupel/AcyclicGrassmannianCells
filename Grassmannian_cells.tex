...
\end{enumerate}
\end{lemma}
For two vertices $p,q\in Q_0$ with $p\in N_q$,  when deleting the unique arrow between $p$ and $q$, we $Q$ splits up into two subquivers which we denote by $K_p$ and $K_q$.
Write for short $N_{i,j}:=N_{p_{i,j}}$
\begin{definition} We call a pair $(E_{i,j}^m,I)$ where $I\subset N_{i,j}$ with $N_{i,j}\backslash I=\{q\}$ \emph{strong reduced}\sayT{maybe we should call it reduced} if the following conditions are satisfied:
\begin{enumerate}
\item $(E_{i,j}^m,I)$ is reduced;
\item If $q,j\in N_p$, then $q$ is not a leaf if $j$ is a limb;
\item If $q\in N_j$, then $K_q$ is not of Dynkin type $D$.
\end{enumerate}
\end{definition}
...
\begin{proposition}Let $Q$ be a tree quiver which is not $3$-valent. Let $i,j\in Q_0$ with $i\neq j$ such that $(E_{i,j}^m,I)$ is strong reduced with $N_{i,j}\backslash I=\{q\}$. Then there exists a vertex $l\in Q_0$ and $J\subset N_{l,q}$ with $i\in J$ and $N_{l,q}\backslash J=\{t\}$ such that $(E^m_{l,q},J)$ is strong reduced.
