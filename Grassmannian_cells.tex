...
\section{Truncated preprojective representations of tree quivers}
  ...
  \begin{lemma}
    \sayT{ statement should be correct now, proof must be adjusted}
    ...
    Then $P_{i_k,m_k}/P_{i_0,m_0}$ is preinjective if and only if $i_1$ is a limb with respect to $i_0$ or if there exists at most one vertex $i_s$ for $s$ for $s=1,\ldots, k-1$  which is not a limb such that the following conditions hold. 
		\begin{enumerate}
		\item We have $N_{i_s}=\{i_{s-1},i_{s+1}, q\}$;
		\item If $s=1$, then $i_k$ is a leaf and $q$ is a limb;
		\item If $s=2$, then $i_k$ and $q$ are both leaves.
		\item If $s=3$, then $Q$ has precisely is a flag quiver with precisely one $3$-valent vertex and, furthermore, $q$ and $i_k$ are leaves and $k=5$. 
		\end{enumerate}
\end{lemma}
\begin{proof}
...
Recall that if $\delta$ is regular with $\Sc{\delta}{\alpha}<0$, we have $\Ext(M_\delta,M_\alpha)\neq 0$ for all representations of dimension $\delta$ and $\alpha$, and thus $\alpha$ cannot be a preinjective root.
  ...
  $$|Q_0(-,i_s)\backslash\{i_{s+1}\}\cup Q_0(-,i_t)\backslash\{i_{t+1}\}|\geq 2.$$
  Let $\{q_1,q_2\}\in Q_0(-,i_s)\backslash\{i_{s+1}\}\cup Q_0(-,i_t)\backslash\{i_{t+1}\}$.
  If $s,t\neq k$, then  $\delta=\sum_{j=0}^ki_j+\sum_{j=s}^ti_j+q_1+q_2$ is a regular root such that $\Sc{\delta}{\alpha}=\delta_1-\delta_{q_1}-\delta_{q_2}<0$.
...
%similar to the proof of \ref{pro:simpleregular} it is straightforward to construct a regular representation $M$ of dimension $\delta$ with $\Hom(P_{i_k}/P_{i_0},M)\neq 0$ which yields that $P_{i_k}/P_{i_0}$ is not preinjective.
  ...
  If $i_k$ is not a limb, then $S_{i_k}$ is a quotient of $P_{i_k}/P_{i_0}$ which is regular by Proposition \ref{pro:simpleregular}. Thus $P_{i_k}/P_{i_0}$ is not preinjective.
...
If $q_1$ and $i_k$ are leaves and $s=k-1$, it is again straightforward to check that $P_{i_k}/P_{i_0}$ is preinjective as it can be transformed into $S_s$ (with $s$ having one ingoing arrow) by reflection functors. A similar argument shows that this is true when $s=3$, $k=5$ and $q_1$ is a leaf and $Q$ has only one $3$-valent vertex. The same is true if $s=2$ and $q_1$ is a leaf.

If $i_1$ is a limb with respect to $i_0$ (whence $i_k$ is a limb, but not necessarily a leaf), as in the preceding case, it can be checked straightforwardly that $P_{i_k}/P_{i_0}$ is preinjective as it can be transformed to a simple preinjective representation.\sayT{do this direct check?}
%Thus if
...
%the representation $P_{i_k}/P_{i_0}$ is preinjective if and only if the single neighbor is a limb.
 ...
 Consider the case when $c=2$, $2 \leq s\neq k$ arbitrary, i.e. $t=k$, $q_1$ is a limb with respect to $i_s$ and $q_2$ is a limb with respect to $i_k$. 
As $Q$ is wild or of extended Dynkin type, there exists a regular root with $\delta_1<\delta_{q_1}+\delta_{q_2}$ and thus $\alpha$ can only be preinjective if either $i_k$ is a leaf or $i_s$ and thus $i_1$ is a limb with respect to $i_0$. As we already considered the last case, we consider the first one.

 

If $Q=\tilde D_n$, $\alpha$ is preinjective as $i_k$ and $q_1$ are forced to be leaves and as $k=s+1$.

If $ Q=\tilde E_6$, the root $\alpha$ is regular as $\Sc{\delta}{\alpha}=0$, see \cite[Section 7, Lemma 2]{CB} for the argument and \cite[Section 4]{CB} for a list of the imaginary Schur roots of extended Dynkin quivers.
If $Q=\tilde E_7$, the same argument shows that the root $\alpha$ is regular if $k=3$ and $s=2$ and preinjective if $k=5$ and $s=2$ (which are the only possibilities).
...
Thus assume that $k=5$ and that $Q$ is not $3$-valent...
