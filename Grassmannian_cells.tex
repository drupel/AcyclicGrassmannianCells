...
\end{lemma}
\begin{proof}
Recall that $\dim (P_{\ell,0})_n$ is the number of paths from $\ell$ to $n$. As there is an arrow from $n$ to $\ell$ and as $Q$ is acyclic, we have $\dim (P_{\ell,0})_n=0$.
...
If $\dim\Ext(S_n,P_{\ell,0})\geq 2$, it follows that $\udim E$ is an imaginary Schur root. This means that $E$ is not preinjective.

Thus assume that $\dim\Ext(S_n,P_{\ell,0})\geq|Q_1(n,\ell)|=1$. We have that $\ell$ is the only vertex in the support of $P_{\ell,0}$ which is a neighbor of $n$ because otherwise we could again conclude that $\dim\Ext(S_n,P_{\ell,0})\geq 2$. But this already shows that 
$\Sigma_n\Sigma_{\ell-1}\cdots\Sigma_{1} E\cong S_\ell$. As $\ell$ is not a limb vertex, it has at least one neighbor except $n$ which is by construction a source of $\mu_n\mu_{\ell-1}\cdots\mu_{1} Q$. As $n$ was a source of $Q$, it follows that $n$ is a sink of $\mu_n\mu{\ell-1}\cdots\mu_{1} Q$. In turn $\ell$ is neither sink nor source and thus $S_{\ell}$ is not preinjective because we assumed that $Q$ is not of Dynkin type.
