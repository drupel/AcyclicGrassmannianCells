...
\begin{figure}
  \[
    \xymatrixcolsep{0.6pc}\xymatrix{
      & & & & & P_{i,m} \ar[dr]\ar[r] & \cdots \ar[r] & P_{i,m+1} \ar[dr]\ar[r] & \cdots \ar[r] & \cdots \ar[dr]\ar[r] & \cdots \\
      & & & & P_{i_t,m_t} \ar[ur]\ar[dr] & & P_{i_t,m^*_t} \ar[ur]\ar[dr] & & \cdots \ar[ur]\ar[dr] & & \cdots \\
...
\begin{proposition}
  ...
  Enumerate the vertices $Q(\ell)_0:=\{i_0=\ell',\ldots,i_t=\ell,i_{t+1}=i\}$ of the type $A$ component associated to $\ell$ so that $|Q_1(i_l,i_{l+1})\sqcup Q_1(i_{l+1},i_l)|=1$ for $0\le l\le t$.
  Given $m\ge0$, set $m_{t+1}:=m=:m^*_{t+1}$ and recursively define
  \[
    m_{r-1}:=
    \begin{cases}
  ...
  \]
  \begin{enumerate}
    \item Given $m\ge0$, $0\le r\le t$, and $t-r\le s\le t$, if $m_r\ge0$, then we have an exact sequence
      \begin{equation}
        \label{eq:limb sequence}
        \ses{P_{i_r,m_r}}{P_{i,m} \oplus P_{i_{r+s-t-1},m^{**}_{r+s-t-1}}}{P_{i_s,m^*_s}},
      \end{equation}
      where $m^{**}_{r+s-t-1}=m_{r+s-t-1}+(t+1-s)=m^*_{r+s-t-1}-(t+1-r)$ and the second summand in the middle is omitted when $r+s=t$.
    \item Given $m\ge0$ and $0\le r\le t$, if $m_r\ge0$, then we have an exact sequence
      \begin{equation}
        \label{eq:thin limb sequence}
        \Ses{P_{i_r,m_r}}{P_{i,m}}{P_{i_{t-r},m^*_{t-r}}}{f}{g}.
      \end{equation}
      Moreover, $f=:f_{i,i_r}^{(m)}$ is a sectional monomorphism and $g=:g_{i_{t-r},i}^{(m)}$ is a sectional epimorphism.
      That is, if $m_r\ge0$, then 
      \begin{itemize}
        \item there exists a filtration
          \[P_{i_r,m_r} \subset P_{i_{r+1},m_{r+1}} \subset \cdots \subset P_{i_t,m_t} \subset P_{i,m}\]
          with each inclusion $P_{i_s,m_s}\into P_{i_{s+1},m_{s+1}}$ for $r\le s\le t$ an irreducible morphism factoring $f_{i,i_r}^{(m)}$;
        \item there exists a string of epimorphisms
          \[P_{i,m} \onto P_{i_{t-r},m^*_{t-r}} \onto \cdots \onto P_{i_1,m^*_1} \onto P_{i_0,m^*_0}\]
          with each surjection $P_{i_s,m^*_s}\onto P_{i_{s-1},m^*_{s-1}}$ for $1\le s\le t-r$ an irreducible morphism factoring $g_{i_{t-r},i}^{(m)}$.
      \end{itemize}
    \item Given $m\ge0$, if there exists $0\le r\le t+1$ with $m_{r+1}=0$ and $m_r=-1$, then each sectional morphism $P_{i,m}\to P_{i_s,m^*_s}$ is injective for $t-r\le s\le t$ and in this case there is a short exact sequence
      \[\ses{P_{i,m}}{P_{i_s,m^*_s}}{I_{i_{t-s},m_{t-s}}}.\]
      %there exists a filtration
      %\[P_{i,m} \subset P_{i_t,m^*_t} \subset P_{i_{t-1},m^*_{t-1}} \subset \cdots \subset P_{i_{t-r},m^*_{t-r}}\]
      %with each inclusion $P_{i_{s+1},m^*_{s+1}}\into P_{i_s,m^*_s}$ for $t+1-r\le s\le t$ an irreducible morphism.
      %Moreover, in this case, we have $P_{i_s,m^*_s}/P_{i_t,m^*_t} \cong I_{i_{t-s-1},-m_{t-s-1}-1}$ is preinjective for $t+1-r \le s < t \le t+1$.
  \end{enumerate}
\end{proposition}
\begin{remark}
  It may be useful to note that $m^*_0-m_0=h(\ell)+h'(\ell)=t+1$.
\end{remark}
\begin{proof}
  We establish (1) by simultaneous induction on $t$ and reverse induction on $|r+s|$.
  When $r=t=0$, we must have $s=0$ and $i_0=\ell=\ell'$. 
  ...
  More generally, when $r=s=t$, for $m\ge0$ with $m_t\ge0$, Lemma~\ref{le:standard AR sequences} gives the desired exact sequence
  \[\ses{P_{i_t,m_t}}{P_{i,m} \oplus P_{i_{t-1},m_{t-1}+1}}{P_{i_t,m^*_t}}.\]
  
  By induction, for $0\le r\le t$ and $t-r\le s\le t$, we have exact sequences 
  \[\ses{P_{i_r,m_r}}{P_{i,m} \oplus P_{i_{r+s-t},m_{r+s-t}+(t-s)}}{P_{i_{s+1},m^*_{s+1}}}\]
  and
  \[\ses{P_{i_{r+s-t},m_{r+s-t}+(t-s)}}{P_{i_{s+1},m^*_{s+1}} \oplus P_{i_{r+s-t-1},m_{r+s-t-1}+(t+1-s)}}{P_{i_s,m^*_s}}\]
  which combine using Corollary~\ref{cor:sequence reducing} to give \eqref{eq:limb sequence} with $m^{**}_{r+s-t-1}=m_{r+s-t-1}+(t+1-s)$.
  Similarly, for $0\le r\le t$ and $t-r\le s\le t$, we have by induction exact sequences 
  \[\ses{ P_{i_r,m_r} }{ P_{i_{r+1},m_{r+1}} \oplus P_{i_{r+s-t-1},m^*_{r+s-t-1}+(t+1-r)} }{ P_{i_{r+s-t},m^*_{r+s-t}+(t-r)} }\]
  and
  \[\ses{P_{i_{r+1},m_{r+1}}}{P_{i,m} \oplus P_{i_{r+s-t},m^*_{r+s-t}+(t-r)}}{P_{i_s,m^*_s}}\]
  which again combine using Corollary~\ref{cor:sequence reducing} to give \eqref{eq:limb sequence} with $m^{**}_{r+s-t-1}=m^*_{r+s-t-1}-(t+1-r)$.


  Part (2) is the special case of (1) when $r+s=t$, the statements about sectional morphisms are immediate from the construction of the sequences \eqref{eq:thin limb sequence}.

  To see part (3)\dots
  \sayD{I can see this most clearly using the derived category, is there a clean way to not use it?}
\end{proof}
...
\begin{figure}
  \[
    \xymatrixcolsep{0.6pc}\xymatrix{
      ...
      & & & \cdots \ar[ur]\ar[dr] & & \cdots \ar[ur]\ar[dr] & & \cdots \ar[ur]\ar[dr] & & \cdots \ar[ur]\ar[dr] \\
      & & & & P_{i'_{t'},m'_{t'}} \ar[ur]\ar[dr] & & P_{i'_{t'},{m'}^*_{\! t'}} \ar[ur]\ar[dr] & & \cdots \ar[ur]\ar[dr] & & \cdots \\
...
\begin{proposition}
  For $i\in Q_0$, consider a subset $L=\{\ell,\ell'\}\subset L_-\sqcup L_+$ with $|L|=2$.
  Write 
  \[Q(\ell)_0=\{i_0,i_1\ldots,i_t=\ell,i_{t+1}=i\} \qquad \text{and} \qquad Q(\ell')_0=\{i'_0,i'_1,\ldots,i'_{t'}=\ell',i'_{t'+1}=i\}\]
  for the vertices of the associated limb quivers $Q(\ell)$ and $Q(\ell')$.
  For $m\ge0$, define $m_r$, $m^*_r$ for $0\le r\le t$ (resp. $m'_{r'}$, ${m'}^*_{\! r'}$ for $0\le r'\le t'$) as above for the quiver $Q(\ell)$ (resp. $Q(\ell')$).
  The following hold.
  
  \begin{enumerate}
    \item If $m_r\ge0$ and $m'_{r'}\ge0$, then there exists a filtration
      \[P_{i_r,m_r} \subset P_{i_{r+1},m_{r+1}} \subset \cdots \subset P_{i_t,m_t} \subset P_{i'_{t'-r'},{m'}^*_{\! t'-r'}}\]
      with each inclusion $P_{i_s,m_s}\into P_{i'_{t'-r'},{m'}^*_{\! t'-r'}}$ for $r\le s\le t$ factoring through the projection ${g'}^{(m)}_{i'_{t'-r'},i'_{s'}}:P_{i'_{s'},{m'}^*_{\! s'}}\onto P_{i'_{t'-r'},{m'}^*_{\! t'-r'}}$ for $r'\le s'\le t'$.
      Note that an analogous filtration exists with the roles of $\ell$ and $\ell'$ interchanged.

      In this case, there is an isomorphism $P_{i_{t-r},m^*_{t-r}}/P_{i'_{r'},m'_{r'}} \cong P_{i'_{t'-r'},{m'}^*_{\! t'-r'}}/P_{i_r,m_r}$.
    \item Suppose $|Q_1(i,-)\sqcup Q_1(-,i)|=3$ with $Q_0(i,-)\sqcup Q_0(-,i)=\{\ell,\ell',\ell''\}$.
      Write $Q(\ell'')$ for the longest type $A$ subquiver of $Q$ containing $i$ and $\ell''$ (i.e. each non-leaf vertex of $Q(\ell'')$ has only two adjacent arrows in $Q$ itself).
      Enumerate the vertices of $Q(\ell'')$ as $Q(\ell'')_0=\{i''_0,i''_1,\ldots,i''_{t''}=\ell'',i''_{t''+1}=i\}$ and as above define $m''_{r''}$, ${m''}^*_{\! r''}$ for $0\le r''\le t''$.
      Then
      \begin{enumerate}
        \item \mbox{}
          \begin{enumerate}
            \item If $m_0\ge0$ (resp. $m'_0\ge0$), then there exists a short exact sequence
              \[\ses{P_{i_0,m_0}}{P_{i''_{t''},{m''}^*_{\! t''}}}{P_{i'_0,{m'}^*_{\! 0}+1}}\]
              (resp. a short exact sequence
              \[\ses{P_{i'_0,m'_0}}{P_{i''_{t''},{m''}^*_{\! t''}}}{P_{i_0,m^*_0+1}}\text{).}\]
            \item If $m_0<0$ (resp. $m'_0<0$), then there exists a short exact sequence
              \[\ses{P_{i''_{t''},{m''}^*_{\! t''}}}{P_{i'_0,{m'}^*_{\! 0}+1}}{I_{i_0,m_0}}\]
              (resp. a short exact sequence
              \[\ses{P_{i''_{t''},{m''}^*_{\! t''}}}{P_{i_0,m^*_0+1}}{I_{i'_0,m'_0}}\text{).}\]
          \end{enumerate}
        \item \mbox{}
          \begin{enumerate}
            \item If $m_0\ge0$, $t'=0$, and $t''>0$, then there exists a short exact sequence
              \[\ses{P_{i_0,m_0}}{P_{i''_{t''-1},{m''}^*_{\! t''-1}}}{P_{i_0,m^*_0+2}}.\]
            \item If $m_0<0$, $t'=0$, and $t''>0$, then there exists a short exact sequence
              \[\ses{P_{i''_{t''-1},{m''}^*_{\! t''-1}}}{P_{i_0,m^*_0+2}}{I_{i_0,m_0}}.\]
          \end{enumerate}
        \item Suppose $t=t'=0$ (i.e. $\ell$ and $\ell'$ are both leaves).
          \begin{enumerate}
            \item If $m_0\ge0$ (resp. $m'_0\ge0$), then for $0\le r''\le t''$ with $t''-r''$ even there exists a short exact sequence
              \[\ses{P_{i_0,m_0}}{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i'_0,{m'}^*_{\! 0}+t''+1-r''}}\]
              (resp. a short exact sequence
              \[\ses{P_{i'_0,m'_0}}{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i_0,m^*_0+t''+1-r''}}\text{).}\]
            \item If $m_0<0$ (resp. $m'_0<0$), then for $0\le r''\le t''$ with $t''-r''$ even there exists a short exact sequence
              \[\ses{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i'_0,{m'}^*_{\! 0}+t''+1-r''}}{I_{i_0,m_0}}\]
              (resp. a short exact sequence
              \[\ses{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i_0,m^*_0+t''+1-r''}}{I_{i'_0,m'_0}}\text{).}\]
            \item If $m_0\ge0$ (resp. $m'_0\ge0$), then for $0\le r''\le t''$ with $t''-r''$ odd there exists a short exact sequence
              \[\ses{P_{i_0,m_0}}{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i_0,m^*_0+t''+1-r''}}\]
              (resp. a short exact sequence
              \[\ses{P_{i'_0,m'_0}}{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i'_0,{m'}^*_{\! 0}+t''+1-r''}}\text{).}\]
            \item If $m_0<0$ (resp. $m'_0<0$), then for $0\le r''\le t''$ with $t''-r''$ odd there exists a short exact sequence
              \[\ses{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i_0,m^*_0+t''+1-r''}}{I_{i_0,m_0}}\]
              (resp. a short exact sequence
              \[\ses{P_{i''_{r''},{m''}^*_{\! r''}}}{P_{i'_0,{m'}^*_{\! 0}+t''+1-r''}}{I_{i'_0,m'_0}}\text{).}\]
          \end{enumerate}
      \end{enumerate}
    \item Suppose $|Q_1(i,-)\sqcup Q_1(-,i)|\ge4$.
      Then
      \begin{enumerate}
        \item The map $P_{i_t,m_t}\oplus P_{i'_{t'},m'_{t'}}\to P_{i,m}$ is injective.
      \end{enumerate}
    \item there exists an exact sequence 
      \[\ses{P_{i,m}}{P_{i_t,m_t} \oplus P_{i'_{t'},m'_{t'}}}{P_{i,m+1}^{\hat L}}\]
      for the truncated preprojective representation
      \[
...
\end{proposition}
\begin{proof}
  To see the last part of (1), we note that there is a commutative diagram
  \[
    \xymatrix{
      & & 0 \ar[d] & 0 \ar[d] & \\
      & & P_{i'_{r'},m'_{r'}} \ar@{=}[r]\ar[d] & P_{i'_{r'},m'_{r'}} \ar[d] & \\
      0 \ar[r] & P_{i_r,m_r} \ar[r]\ar@{=}[d] & P_{i,m} \ar[r]\ar[d] & P_{i_{t-r},m^*_{t-r}} \ar[r]\ar[d] & 0 \\
      0 \ar[r] & P_{i_r,m_r} \ar[r] & P_{i'_{t'-r'},{m'}^*_{\! t'-r'}} \ar[r]\ar[d] & X \ar[r]\ar[d] & 0 \\
      & & 0 & 0 & 
    }
  \]

  Part (2.a.i) follows from applying Corollary~\ref{cor:sequence reducing} to the sequences
  \[\ses{P_{i,m}}{ P_{i_t,m^*_t} \oplus P_{i'_{t'},{m'}^*_{\! t'}} \oplus P_{i''_{t''},{m''}^*_{\! t''}} }{P_{i,m+1}}\]
  \[\ses{P_{i_0,m_0}}{P_{i,m}}{P_{i_t,m^*_t}}\]
  \[\ses{P_{i'_{t'},{m'}^*_{\! t'}}}{P_{i,m+1}}{P_{i'_0,{m'}^*_{\! 0}+1}}\]
  \sayD{Is there a nice way to see (2.a.ii) without derived categories?}

  Part (2.b.i) follows from applying Corollary~\ref{cor:sequence reducing} once using the sequences
  \[\ses{P_{i,m}}{ P_{i_t,m^*_t} \oplus P_{i'_{t'},{m'}^*_{\! t'}} \oplus P_{i''_{t''},{m''}^*_{\! t''}} }{P_{i,m+1}}\]
  \[\ses{P_{i_0,m_0}}{P_{i,m}}{P_{i_t,m^*_t}}\]
  \[\ses{P_{i''_{t''},{m''}^*_{\! t''}}}{P_{i,m+1} \oplus P_{i''_{t''-1},{m''}^*_{\! t''-1}}}{P_{i''_{t''},{m''}^*_{\! t''}+1}}\]
  and then again using the sequence
  \[\ses{P_{i'_0,{m'}^*_{\! 0}}}{P_{i''_{t''},{m''}^*_{\! t''}+1}}{P_{i_0,m^*_0+2}}\]
  coming from (2.a.i).

  We prove parts (2.c.i) and (2.c.iii) by simultaneous downward induction on $r''$, the base cases $r''=t''$ and $r''=t''-1$ being (2.a.i) and (2.b.i) respectively.
  By iteratively applying Corollary~\ref{cor:sequence reducing}, we may replace the sequence
  \[\ses{P_{i,m}}{ P_{i_0,m^*_0} \oplus P_{i'_0,{m'}^*_{\! 0}} \oplus P_{i''_{t''},{m''}^*_{\! t''}} }{P_{i,m+1}}\]
  by the sequence
  \[\ses{P_{i,m}}{ P_{i_0,m^*_0} \oplus P_{i'_0,{m'}^*_{\! 0}} \oplus P_{i''_{r''},{m''}^*_{\! r''}} }{P_{i''_{r''+1},{m''}^*_{\! r''+1}+1}}\]
  which reduces to the desired sequence by induction using Corollary~\ref{cor:sequence reducing}.
  \sayD{Same question for (2.c.ii) and (2.c.iv).}

\end{proof}



For $i\in Q_0$ and $m\ge0$, given $\ell\in Q_0(-,i)\cup Q_0(i,-)$ we write $\cI_{i,m}^{\hat \ell}\subset\cI_{i,m}$ for the tuple of subspaces with the only proper subspace at position $\ell$ and the zero subspace in that component.
\begin{corollary}
...
\section{Truncated preprojective representations of tree quivers}
  ...
  \noindent In this section we assume that the underlying graph of $Q$ is a connected tree. 
...
\end{proof}
In the following, we call a pair $(i_0,i_k)\in Q_0\times Q_0$ \emph{good} if the quotient $P_{i_k,m_k}/P_{i_0,0}$ is not preinjective, with $m_k$ as defined in Proposition \ref{pro: injective quotients}.


We collect several sufficient conditions for $(i,j)\in Q_0\times Q_0$ being a good pair which follow directly from Proposition \ref{pro: injective quotients}.
\begin{corollary}
Let $(i,j)\in Q_0\times Q_0$.
\begin{enumerate}
\item If $j$ is not a limb, then $(i,j)$ is a good pair.
\item If $j$ is not a leaf and there exist two vertices which are not limbs on the path from $i$ to $j$, then $(i,j)$ is a good pair.
\item If $i$ is a leaf, then $(i,j)$ is a good pair.
\item If $i$ and $j$ are limbs which lie on the same type $A$ subquiver, then $(i,j)$ is not good pair.
...
$$\xi_{i,j}^m:\ses{P_{i,m}}{\bigoplus_{k\in N_{p_{i,j}}}P_{i,k}^m}{P_{i,j}^{m+1}}.$$

\end{lemma}
\begin{proof}
  We proceed by induction on the length of the path. For $i=j$, we have that $\xi_{i,i}^m$ is the Auslander-Reiten sequence considered above. Let $p_{i,j}=i_0\ldots i_n$. In the present notation, the Auslander-Reiten sequences can be written as
...
$$\xi_{i,i_{n-1}}^m:\ses{P_{i,m}}{\bigoplus_{k\in N_{p_{i,i_{n-1}}}}P_{i,k}^m}{P_{i,i_{n-1}}^{m+1}},$$
which exists by induction hypothesis, the second part of Corollary \ref{cor:sequence reducing} gives the claim.
\end{proof}

For two vertices $p,q\in Q_0$ with $p\in N_q$,  when deleting the unique arrow between $p$ and $q$, the quiver $Q$ splits up into two subquivers. We denote the one containing $p$ by $K_{p,q}$ and the one containing $q$ by $K_{q,p}$. Moreover, for a subset $M\subset Q_0$ by $K_M$ we denote the smallest connected subquiver of $Q$ containing $M$. Write for short $N_{i,j}:=N_{p_{i,j}}$.


We call a quiver \emph{$3$-valent} if it is a flag quiver with one $3$-valent vertex.
\begin{definition} Let $i,j,q\in Q_0$. We call $(j,q)$ a dead end with respect to $i$ if one of the following conditions are satisfied:

\begin{enumerate}
\item $j$ and $q$ are both limbs with respect to $i$;
\item there exists a vertex $l\in N_i$ such that 
\begin{enumerate}
\item $j,q\in K_{l,i}$; 
\item $K_{\{j,q,l\}}$  is a $3$-valent quiver;\sayT{delete neighbors of $l$ which are not in direction $j,q$.}
\item $q$ or $j$ is a leaf with respect to the $3$-valent vertex $p$ of $K_{\{j,q,l\}}$.
\end{enumerate}
\end{enumerate}
We call a pair $(\xi_{i,j}^m,I)$ where $I\subset N_{i,j}$ with $N_{i,j}\backslash I=\{q\}$ \emph{strong reduced} if the pair $(j,q)$ is not a dead end with respect to $i$.

\end{definition}

Note that $j$ and $p$ might be equal.

\begin{lemma} \sayT{base cases are missing} If $(j,q)$ is not a dead end with respect to $i$, the pair $(\xi^m_{i,j},N_{i,j}\backslash \{q\})$ is reduced.
\end{lemma}
\begin{proof}
We have to check that $P_{i,q}^0/P_{i,0}$ is not preinjective. If $K_{\{i,j,q\}}$ is not $3$-valent, this follows directly from Proposition \ref{pro: injective quotients}. If $K_{\{i,j,q\}}$ is $3$-valent, by assumption both $q$ and $j$ are no leafs of $K_{l,i}$ where $l\in N_i\cap K_{\{i,j,q\}}$. But then again Proposition \ref{pro: injective quotients} implies that $(\xi^m_{i,j},N_{i,j}\backslash \{q\})$ is reduced.
\end{proof}

\begin{lemma}Let $Q$ be a tree quiver which is not $3$-valent. For every vertex $j\in Q_0$, there exist vertices $i,q\in Q_0$ such that $(\xi_{i,j}^m,N_{i,j}\backslash\{q\})$ is strong reduced.
\end{lemma}
\begin{proof}
If $j$ is a limb with respect to a vertex $l$, then there exists a vertex $i$ such that on $p_{i,j}$ there are at least two $3$-valent vertices $p_1,p_2$ or one $n$-valent vertex $p$ with $n\geq 4$. 

In the first case assume that $d(p_1,j)>d(p_2,j)$ and $q\in N_{p_1}\backslash p_{i,j}$. Then the pair $(j,q)$ is not a dead end with respect to $i$ and thus $(\xi_{i,j}^m,N_{i,j}\backslash\{q\})$ is strong reduced. In the second case $(\xi_{i,j}^m,N_{i,j}\backslash\{q\})$, where $q\in N_p\backslash p_{i,j}$ is arbitrary, is strong reduced.

Thus assume that $j$ is not a limb with respect to any vertex which means that $|N_j|\geq 3$. If $|N_j|=3$, there exists a vertex $i$ which is at least $3$-valent. Taking any vertex $q\in N_i$, it is clear that $(\xi_{i,j}^m,N_{i,j}\backslash\{q\})$ is strong reduced. 

As this arguments also applies if there exist two vertices of valency strictly greater than $2$, it remains to consider the case when $j$ is the only $n$-valent vertex with $n\geq 3$. By assumption we actually have $n\geq 4$ and, moreover, $Q$ is a flag quiver. In this case, taking any $i,q\in N_j$ it follows that $(\xi_{i,j}^m,N_{i,j}\backslash\{q\})$ is strong reduced.
\end{proof}

\begin{lemma}Let $Q$ be a tree quiver which is not $3$-valent. Let $i,j\in Q_0$ with $i\neq j$ such that $(\xi_{i,j}^m,I)$ is strong reduced with $N_{i,j}\backslash I=\{q\}$. Then there exists a vertex $l\in Q_0$ and $J\subset N_{l,q}$ with $i\in J$ and $N_{l,q}\backslash J=\{t\}$ such that $(E^m_{l,q},J)$ is strong reduced.

\end{lemma}

\begin{proof}
We consider the three cases $q\in N_i,\,q\in N_j$ and $q\in N_{i,j}\backslash (N_i\cup N_j)$. Write $i=s_0,s_1,\ldots, s_r=j$ for the vertices on $p_{i,j}$.\sayT{we should make the last two cases into one}

Let $q\in N_i$. Since $(i,q)$ is a good pair, $q$ is not a limb with respect to $i$. Thus there exist $l_1,l_2\in K_{q,i}$ such that $l_1$ and $l_2$ are no limbs with respect to each other (in $Q$). First assume that $l_1\notin N_q$ (or $l_2\notin N_{q}$). As $Q$ is not $3$-valent, Proposition \ref{pro: injective quotients} guarantees that $(l_1,l_2)$ is a good pair. Moreover, $(q,l_2)$ is not a dead end with respect to $l_1$ which means that $(\xi_{l_1,q}^m, N_{l_1,q}\backslash\{l_2\})$ is strong reduced. 

If $(K_{q,i})_0=\{q,l_1,l_2\}$, then again $Q$ being not $3$-valent guarantees that $(q,l_2)$ is not a dead end with respect to $l_1$ which means that $(\xi_{l_1,q}^m, N_{l_1,q}\backslash\{l_2\})$ is also strong reduced.

Let $q\in N_j$. As $(j,q)$ is not a dead end with respect to $i$, we either have $|N_{s_1,q}\cap K_{i,s_1}|\geq 2$ with $\{p_1,p_2\}\in N_{s_1,q}\cap K_{s_1,s_2}$ or $N_{s_1,q}\cap K_{i,s_1}=\{p\}$ for some vertex $p$ which is not a limb with respect to any vertex. 

If $p_1\in N_{s_1}$ in the first case, it is straightforward that $(\xi_{p_1,q}^m,N_{p_1,q}\backslash\{p_2\})$ is strong reduced as $(j,q)$ is not a dead end with respect to $i$ and thus also $(q,p_2)$ is not a dead end with respect to $p_1$. Note that we also have $i\in N_{p_1,q}$ in this case.

If $N_{s_1}=\{i,s_2\}$, then $(\xi_{s_1,q}^m, N_{s_1,q}\backslash\{p_1\})$ is strong reduced as $(j,q)$ is not a dead end with respect to $i$.  

In the second case, the pair $(\xi_{p,q}^m,N_{p,q}\backslash\{l\})$ where $l\in N_{p}\backslash\{s_1\}$ is obviously strong reduced as $(q,l)$ is not a dead end with respect to $p$.

%Thus either $q$ is not a limb with respect to $j$ or there exists $t\in N_{j}$ with $t\neq q$. In the second case, $t$ is not a leaf if $q$ is a leaf because $(i,q)$ is a good pair (part (4) of Proposition \ref{pro: injective quotients}). If $q$ is not a limb with respect to $j$, then for $t\in N_q\backslash\{j\}$ arbitrarily, the pair $(s_1,t)$ is good as $t$ is not a limb with respect to $s_1$ and as $K_q$ is not of type $D$. Moreover, $K_t$ is not of type $D$. In summary, $(\xi_{s_1,q}^m, N_{s_1,q}\backslash\{t\})$ is strong reduced. 
  
Finally, let $q\in N_{s_u}$ for some $u\in\{1,\ldots,r-1\}$. As above first assume that we find $\{p_1,p_2\}\in  N_{s_1,q}\cap K_{s_1,s_2}$. If $p_1\in N_{s_1}$, the pair $(\xi_{p_1,q}^m,N_{p_1,q}\backslash\{p_2\})$ is strong reduced as $(j,q)$ is not a dead end with respect to $i$. Furthermore, if $N_{s_1}=\{i,s_2\}$, then $(\xi_{s_1,q}^m, N_{s_1,q}\backslash\{p_2\})$ is strong reduced.

If $N_{s_1,q}\cap K_{i,s_1}=\{p\}$, then $p$ is not a limb with respect to any vertex. As above $(\xi_{p,q}^m,N_{p,q}\backslash\{l\})$ where $l\in N_{p}\backslash\{s_1\}$ is strong reduced as $(q,l)$ is not a dead end with respect to $p$.
%If $u=r-1$, then $q$ is not a leaf if $j$ is a limb which also ensures that $K_{s_{u}}$ is not of type $D$ in this case. Then $(\xi_{s_1,q}^m, N_{s_1,q}\backslash\{j\})$ is strong reduced as $(s_1,j)$ is a good pair as $j$ is not a limb or $q$ not a leaf if $j$ is a limb.\sayT{DT condition}
...
Finally, $(N(P_{i,k_l^s}),P_{i,k_l^s})$ is also compatible by assumption.\sayT{why can we assume this? We need that this comes from a strong reduced pair which seems to be clear as we can take the path to $i$ or $j$}.

\end{proof}

\section{Side results on tree quivers}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{lemma}\sayT{this holds for arbitrary orientations, check statement, do we need this?}
	Let $i,j,l\in Q_0$ pairwise different such that $i\in p_{j,l}$ and assume that all arrows are oriented towards $i$.  Then $(P_{j,0}\oplus P_{l,0})/P_{i,0}$ is not preinjective and thus $P_{i,m}\to P_{j,m}\oplus P_{l,m}$ is injective for all $m\geq 0$ if and only if $i$ is $3$-valent, $j$ and $k$ are leaves and, moreover, $Q$ is flag quiver.
\end{lemma}
\begin{proof}
If $i$ is the only $3$-valent vertex of a flag quiver and $j$ and $k$ are leaves of $Q$, we have 
$$\alpha:=\udim (P_{j,0}\oplus P_{l,0})/P_{i,0}=\sum_{q\in p_{l,k}}q.$$
By successively reflecting at all sinks on this path (i.e. all vertices on $p_{l,k}$ but $i$), the quotient can be transformed into $S_i$ with one ingoing arrow. Proposition \ref{pro:simpleregular} together with the assumptions yields the claim.  

Thus assume that $\alpha$ is preinjective. If $i$ is $n$-valent with $n\geq 4$ or if there exists $k\in N_i\backslash\{j,l\}$ which is not limb, then the same argument shows that $\alpha$ is regular as it is regular as a representation of its support.  Thus we can assume that $i$ is either $2$-valent or $3$-valent with $k\in N_i\backslash\{j,l\}$ being a limb with respect to $i$.
There is an exact sequence
$$\ses{P_{j,0}}{(P_{j,0}\oplus P_{l,0})/P_{i,0}}{P_{l,0}/P_{i,0}}.$$
If the representation on the right hand side is regular, the middle term cannot be preinjective. This shows that both $(i,j)$ and $(i,k)$ cannot be good pairs if the quotient is preinjective.

Let $j_1\in N_i\cap p_{j,i}$ and $k_1\in N_{i}\cap p_{k,i}$. If $j_1$ is not a limb, then $j$ is a leaf and the same holds for $k_1$ and $k$.
...
\begin{lemma}
  \label{lem:quotients projectives2}
...
\begin{corollary}
  \sayT{we should try to make one statement out of the first two}
  \label{cor:injsur}
  \mbox{}
  \begin{enumerate}
...
\item\sayT{assumptions missing} Let  $i\in Q_0$ and $I\subset N_i$ with $|I|\leq |N_i|-2$. Then the map
...
\begin{comment}
\begin{lemma}
  Let $i\in Q_0$ be not a limb. The pair $(\xi_{i,j}^m,I)$ with $I\neq\emptyset$ is reduced if $|I|\leq |N_{p_{i,j}}|-2$ or if $I=|N_{p_{i,j}}|-1$ and $q\in N_{p_{i,j}}\backslash I$ is not a limb with respect to $i$. 
\end{lemma}      
\begin{proof}
  We proceed by induction on the length of $p_{ij}$. If $i=j$,

The pair $(\xi_{i,j}^m,I)$ is reduced if $\sum_{k\in I}\udim P_{i,k}^m<\udim P_{i,j}^{m+1}$ which is if and only if 
...
\item If $j$ is not a leaf, there exists a vertex $i$ such that the exact sequence $\xi_{i,j}^m$ is reduced.
...
\sayT{easier proof?}
  ...
  \[\xymatrix{
      ...
      & 0 & 0 & }\]
  in which an easy diagram chase shows that the right hand vertical morphism is an isomorphism.
  As the exact sequence
	$$\ses{M_{j+1}/M_j}{M_k/M_j}{M_k/M_{j+1}}$$
  is $\tau$-stable and as $\Hom(M_j/M_i,\tau(M_k/M_j))=\kk$, we end up with an exact sequence
