...
\begin{proposition}
  ...
  Then $S_i$ is preinjective if one of the following cases holds: 
  \begin{enumerate}
    \item $|Q_1(-,i)|=0$;
    \item $|Q_1(-,i)|=1$ and removing all arrows $\alpha\in Q_1(i,-)$ creates a disconnected component $Q'$ containing $i$ which is of type $A$.
      \sayT{to get if and only if we probably have to go for Dynkin type here}
  \end{enumerate}
  Moreover, $S_i$ is not preinjective if $|Q_1(-,i)|\geq 2$.
\end{proposition}
\begin{proof}
