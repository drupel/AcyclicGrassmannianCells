...
\begin{proposition}
  ...
  \begin{enumerate}
    \item $|Q_1(-,i)|=0$;
    \item $|Q_1(-,i)|=1$ and $\ell\in Q_0(-,i)$ is a limb vertex such that the associated leaf is not equal to $i$.\sayT{If the quiver is of extended Dynkin type $A$, the simples corresponding to vertices which are neither sink nor source are regular, but are limb vertices. Probably we want to exclude this cases by definition?!}
		\end{enumerate}
