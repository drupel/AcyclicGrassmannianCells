...
\begin{proof}
  ...
  Now Lemma \ref{lem: non-preinjective} gives the claim. 
 ...
 % If there exists $q\in Q_0(-,i)$ with $|Q_1(q,i)|\ge2$, then the root $q+i$ is imaginary with $\hom(i,q+i)=1$ and thus $S_i$ is not preinjective by Lemma~\ref{lem: non-preinjective}.
  ...
  First assume that $|Q_0(-,i)|\geq 2$.
  Then there exist distinct vertices $q,q'\in Q_0(-,i)$.
  Note that $q,q'\neq j$ as $Q$ is acyclic. 
  \sayD{With the last edits we lost the definition of vertex $j$.}
