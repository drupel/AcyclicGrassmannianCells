...
\begin{proof}
  ...
  For the remaining cases, the aim is to apply Lemma~\ref{lem: non-preinjective} to see that $S_i$ is not preinjective. 
  Let $Q'$ be the connected component containing $i$ after removing all arrows from $Q_1(i,-)$.

  If $Q'$ is wild or of extended Dynkin type, let $M$ be any indecomposable representation of $Q'$ such that $\beta=\udim M$ is an imaginary root of $Q'$ and thus of $Q$.
  We can naturally identify $M$ with a representation of $Q$ by setting $M_\alpha=0$ for all $\alpha\in Q_1(i,-)$.
 ...
 % If there exists $q\in Q_0(-,i)$ with $|Q_1(q,i)|\ge2$, then the root $q+i$ is imaginary with $\hom(i,q+i)=1$ and thus $S_i$ is not preinjective by Lemma~\ref{lem: non-preinjective}.
  ...
  First assume that $|Q_0(-,i)|\geq 2$, i.e.~there exist distinct vertices $q,q'\in Q_0(-,i)$.
...
Assume that 
$$\bigoplus_{h\in H} P_{h,m}\to P_{t,m}$$ is surjective. Then $|H|=|Q_0(t,-)|-1$, the unique vertex $h'\in Q_0(t,-)\backslash\{H\}$ is a limb vertex and we have $P_{t,H}^m=P_{t',m}$ where $t'\in Q_0(-,h')$ with $t'\neq t$. Finally, we obtain a short exact sequence
...
\begin{theorem}
Let $t\in \cT$.
\begin{enumerate}\item If each $h\in Q_0(t,0)=\{h_1,\ldots,h_n\}$ is not a limb vertex, writing $H_i:=\{h_1,\ldots,h_i\}$, for every $m\geq 1$, there exists chains of  non-preinjective exceptional representations
...
\item  Let $Q_0(t,0)=\{h_1,\ldots,h_n\}$ and write $H_i:=\{h_1,\ldots,h_i\}$. Moreover, assume that $h_{i_1},\ldots,h_{i_r}$ are limb vertices. For every $m\geq 1$, there exists chains of regular representations
