...
\begin{lemma}Let $\cF(M)$ be an irreducible orthogonal $\tau$-stable filtration. We have:
\begin{enumerate}
\item The induced map $\phi_i:M_i\to \tau(M_{i+1}/M_i)$ is surjective with $\ker(\phi_i)=M_{i-1}\oplus \tau N_i$.
\item Let $V\subset M_r$ be an exceptional\sayT{do we need this?} subrepresentation with $\Hom(V,M_r)=\kk$ such that $V\cap M_{r-1}=\{0\}$ and such that $(V,M_i)$ is orthogonal for each $i=1,\ldots,r-1$. For $i\leq r-1$ there exist unique (up to scalars) non-zero induced maps $\psi_i:M_i/M_{i-1}\to\tau (M_r/(V\oplus M_{i}))$, for which we have $\im(\psi_i)=\tau(M_{i+1}/M_i)$ and $\ker(\psi_i)=\tau(N_i)$. 
...
$K=M_{i-1}\oplus \tau N_i$, the first part follows by Lemma \ref{lem:mochain}. 
 ...
 As $V\cap M_{r-1}=\{0\}$, there is an exact sequence
