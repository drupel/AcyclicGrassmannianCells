...
\begin{corollary}
  ...
  \begin{enumerate}
    ...
    \item Given $m\ge0$ and $0\le r\le k$, if $m_r\ge0$, then we have an exact sequence
      ...
      \end{itemize}
    \item Given $m\ge0$, if there exists $0\le r\le k$ with $m_r=0$ and $m_{r-1}=-1$, then there exists a filtration
      \[P_{i,m} \subset P_{i_k,m'_k} \subset P_{i_{k-1},m'_{k-1}} \subset \cdots \subset P_{i_{k+1-r},m'_{k+1-r}}\]
      with each inclusion $P_{i_{s+1},m'_{s+1}}\into P_{i_s,m'_s}$ for $k-r+1\le s\le k$ an irreducible morphism.
      Moreover, in this case, each quotient $P_{i_s,m'_s}/P_{i_{s+1},m'_{s+1}}$ for $k-r+1\le s\le k$ is preinjective.
  \end{enumerate}
  ...
  %      \sesM{P_{\ell',m-h(\ell)}}{P_{i,m}}{P_{\ell,m+1}}{f}
  %    \end{equation}
  %    with $f=:f_{i,\ell}^m$ a sectional monomorphism.
  %  \item If $\ell\in Q_0(-,i)$, then for any $m\ge h(\ell)$, we have an exact sequence
  %    \begin{equation}
  %      \label{eq:thin source sequence}
  %      \sesM{P_{\ell',m-h(\ell)}}{P_{i,m}}{P_{\ell,m}}{f}
  %    \end{equation}
  %    with $f$ a sectional monomorphism.
  %\end{enumerate}
\end{corollary}
\begin{remark}
...
\begin{proof}
  ...
  which combine using Lemma~\ref{le:sequence reducing} to give \eqref{eq:limb sequence}.
...
\begin{lemma}
  Suppose $i\in Q_0$ is adjacent to a limb vertex $\ell$ with associated leaf $\ell'$.
  For $m\ge h(\ell)$, the representation $P_{i,m}/P_{\ell',m-h(\ell)}$ is preinjective if and only if 
\end{lemma}
\begin{proof}
\end{proof}
...
\section{Truncated preprojective representations of tree quivers}
  ...
  \begin{lemma}
    ...
    Then $P_{i_k,m_k}/P_{i_0,m_0}$ is preinjective if and only if $i_1$ is a limb with respect to $i_0$ or if there exists at most one vertex $i_s$ for $s=1,\ldots, k$  which is not a limb with $N_{i_s}=\{i_{s-1},i_{s+1}, q\}$  such that one of the following conditions holds. 
		\begin{enumerate}
		
		\item If $s=1$, then $i_k$ is a leaf and $q$ is a limb;
		\item If $s=2$, then $i_k$ and $q$ are both leaves.
		\item If $s=3$, then $Q$ has is a flag quiver with precisely one $3$-valent vertex and, furthermore, $q$ and $i_k$ are leaves and $k=5$. 
		\item If $s=k-1$, then $q$ and $i_k$ are leaves.
		\item If $s=k$, then $i_1$ is a limb and $q=i_{k+1}$.
		\end{enumerate}
\end{lemma}
\begin{proof}
  We may without loss of generality assume that that all arrows are oriented towards $i_0$.
	  If $i_k$ is not a limb, then $S_{i_k}$ is a quotient of $P_{i_k}/P_{i_0}$ which is regular by Proposition \ref{pro:simpleregular}. Thus $P_{i_k}/P_{i_0}$ is not preinjective. Thus in the following assume that $i_k$ is limb.

	  In particular, we have $i_j\in Q_0(-,i_{j-1})$ for $j=1,\ldots,k$, $m_k=0$ and
  $$\alpha:=\udim P_{i_k}/P_{i_0}=\sum_{j=1}^{k}i_j.$$
	For any root $\delta$, we have 
$$\Sc{\delta}{\alpha}=\sum_{j=1}^k\delta_i-\sum_{j=1}^k\sum_{l\in Q(-,j)}\delta_l.$$
Recall that if $\delta$ is regular with $\Sc{\delta}{\alpha}<0$, we have $\Ext(M_\delta,M_\alpha)\neq 0$ for all representations of dimension $\delta$ and $\alpha$, and thus $\alpha$ cannot be a preinjective root.

	Assume that there are two vertices $i_s$ and $i_t$ with $1\leq s,t\leq k-1$ and
  ...
  If $s,t\neq k$, then  $\delta=\sum_{j=0}^ki_j+\sum_{j=s}^ti_j+q_1+q_2$ is a regular root such that $\Sc{\delta}{\alpha}<0$.
	
	

%similar to the proof of \ref{pro:simpleregular} it is straightforward to construct a regular representation $M$ of dimension $\delta$ with $\Hom(P_{i_k}/P_{i_0},M)\neq 0$ which yields that $P_{i_k}/P_{i_0}$ is not preinjective.


Thus assume that $t=k$ and $s\neq k$. The unique exceptional representation of dimension 
$\alpha:=\sum_{j=s}^ki_j$ is a factor of $P_{i_k}/P_{i_0}$. Consider the full subquiver $Q'$ with vertices $Q_0\backslash\{i_{k+1},\ldots,i_n\}$. When reflecting successively at the sources $i_k,\ldots,i_{s-1}$ the representation $P_{i_k}/P_{i_0}$ transforms into the simple representation $S_s$ of the corresponding transformation of $Q'$. By Proposition \ref{pro:simpleregular}, the representation $S_s$ is a preinjective representation of $Q'$ if and only if $q_1$ is a limb with respect to $s$. By Lemma \ref{lem:regular support} and the argument from above, this shows that $P_{i_k}/P_{i_0}$ is regular if $q_1$ is not limb. Thus we have shown so far that $\alpha$ can only be preinjective if $c=2$, $q_1$ is a limb with respect to one of the vertices $i_1,\ldots, i_{k-1}$ and $q_2$ is a limb with respect to $i_k$. In the following we assume that this is the case.

First assume that $s=1$. The preceding argument shows that $P_{i_k}/P_{i_0}$ is preinjective if $i_k$ is a leaf and $q_1$ is a limb with respect to $i_1$. 

If $q_1$ and $i_k$ are leaves and $s=k-1$, it is again straightforward to check that $P_{i_k}/P_{i_0}$ is preinjective as it can be transformed into $S_s$ (with $s$ having one ingoing arrow) by reflection functors.

A similar argument shows that this is true if $s=3$, $k=5$ and $q_1$ is a leaf and $Q$ has only one $3$-valent vertex and, furthermore, if $s=2$ and $q_1$ is a leaf. 

If $i_1$ is a limb with respect to $i_0$ (whence $i_k$ is a limb, but not necessarily a leaf), as in the preceding case, it can be checked straightforwardly that $P_{i_k}/P_{i_0}$ is preinjective as it can be transformed to a simple preinjective representation.\sayT{details}
%Thus if
...
It remains to show that $\alpha$ is regular in the remaining cases where we may assume that $c=2$, $2 \leq s\neq k$ arbitrary, i.e. $t=k$, $q_1$ is a limb with respect to $i_s$ and $q_2$ is a limb with respect to $i_k$. Then we have
$$\Sc{\delta}{\alpha}=\delta_{i_1}-\delta_{q_1}-\delta_{q_2}.$$

As $Q$ is wild or of extended Dynkin type such that $i_k$ is not a leaf and $i_s$ (and thus $i_1$) is not a limb with respect to $i_0$, there exists a regular root with $\delta_1<\delta_{q_1}+\delta_{q_2}$. This is rather straightforward as $Q$ has a subquiver of extended Dynkin type $\tilde E_n$ which includes all vertices under consideration and thus the unique imaginary Schur root $\delta$ satisfies the condition, see \cite[Section 7, Lemma 2]{CB} for a list. 

Thus $\alpha$ can only be preinjective if $i_k$ is a leaf or $i_1$ is a limb with respect to $i_0$.  As we already considered the last case, taking into account the observations from above we have to deal with the case that $c=1$, $k>s\geq 2$, $q=q_1$ is a limb with respect to $i_s$ and $i_k$ is a leaf. Then we have 
$$\Sc{\delta}{\alpha}=\delta_{i_1}-\delta_q.$$
Moreover, we can assume that $q$ is not a leaf if $s=k-1$ or $s=2$. If $Q'$ is the minimal
 which means that there exists no subquiver $Q'$ of extended Dynkin type including the vertices $i_k$ and $q$ which is of type $D$.\sayT{do we need this remark?} 
If $Q$ has a $n$-valent vertex other than $i_s$ with $n\geq 3$, then it is straightforward to check that  there exists a regular root which satisfies this condition. \sayT{carry this out}

Thus assume that $i_s$ is the only $n$-valent vertex of $Q$ with $n\geq 3$. Then $Q$ has a subquiver which is of extended Dynkin type $\tilde E$. We first assume that $Q$ itself is of extended Dynkin type.\sayT{continue here, the rest are notes which are maybe not entirely correct}
...
\end{corollary}
\begin{proof} 
  The statement follows from Lemma \ref{lem:irreducible morphisms}. 
