...
If we write $V_u=\bigoplus_{l=u}^rP_{i,k_l}^m$ for $u=2,\ldots, r$, we may consider the exact sequences\say{make a choice such that the cokernel is not zero for $l=1$ and $s=t_l$?!}
...
where $N(P_{i,k_l^s})$ are direct summmands of the middle terms of the Auslander-Reiten sequence 
...
\begin{lemma}\label{lem:mochain2}\sayT{the case $i=j$ is different and must be treated}
