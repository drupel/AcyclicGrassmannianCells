...
\begin{corollary}
  ...
  Enumerate the vertices $Q(\ell)_0:=\{\ell'=i_0,\ldots,i_{h(\ell)-1}=\ell,i_{h(\ell)}=i\}$ of the type $A$ component associated to $\ell$ so that $|Q_1(i_l,i_{l+1})\sqcup Q_1(i_{l+1},i_l)|=1$ for $1\le l\le k$.  
  \begin{enumerate}
    \item If $\ell\in Q_0(i,-)$, then for any $m\ge 1$ and $[h(\ell)-m]_+ \le r < h(\ell)$, we have an exact sequence
      \begin{equation}
        \label{eq:thin limb sequence 1}
        \sesM{P_{i_r,m_r}}{P_{i,m}}{P_{i_{h(\ell)-1-r},m_r+h(\ell)+1}}{f}
      \end{equation}
      where $m_0=m-h(\ell)$ and 
      $m_{r+1}=
        \begin{cases}
          m_r & \text{if $i_{r+1} \to i_r$;}\\
          m_r+1 & \text{if $i_r \to i_{r+1}$.}
        \end{cases}$
...
\end{lemma}
\begin{proof}
  We proceed by induction on the length of the path. For $i=j$, we have that $E_{i,i}^m$ is the Auslander-Reiten sequence considered above. Let $p_{i,j}=i_0\ldots i_n$. In the present notation, the Auslander-Reiten sequences can be written as
