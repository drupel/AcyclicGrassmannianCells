...
\item Let $j\in Q_0(i,-)$. Then $P_{j,1}/P_i$ is preinjective if and only if $j$ is a limb with respect to $i$. Furthermore, we have $\tau^{-h(j)-1}(P_j/P_i)=0$ in this case.
\end{enumerate}
\end{lemma}
...
\end{proof}
\begin{remark}Let $i\in Q_0(j,-)$ (resp. $i\in Q_0(-,j)$). Note that if $i$ and $j$ are both limbs $P_{i,m}\to P_{j,m}$ (resp. $P_{i,m-1}\to P_{j,m}$) becomes surjective for big enough $m$ if and only if $j$ is closer to the leave of the corresponding subquiver of type $A$. The maps $P_{j,m}\to P_{i,m+1}$ (resp. $P_{j,m}\to P_{i,m}$) are always injective if $j$ is a limb with respect to $i$.
...
\end{lemma}      
\begin{proof}The pair $(E_{i,j}^m,I)$ is reduced if $\sum_{k\in I}\udim P_{i,k}^m<\udim P_{i,j}^{m+1}$ which is if and only if 
