...
\begin{proposition}
  ...
  \begin{enumerate}
    \item $|Q_1(-,i)|=0$;
    \item $|Q_1(-,i)|=1$ and removing all arrows $\alpha\in Q_1(i,-)$ creates a disconnected component $Q'$ containing $i$ which is of type $A$.
		\end{enumerate}
...
\begin{proof}
  ...
  Then $S_i=\tau^{-h(\ell)}I_{\ell',0}$ is preinjective.\sayT{reference?}

For the remaining cases, the aim is to apply Lemma~\ref{lem: non-preinjective}. 

First consider the case $|Q_1(-,i)|\geq 2$.
  ...
  If $j$ is connected to $q$ (or analogously to $q'$) by a sequence $j=q_1,q_2,\ldots,q_r=q$ with $q_i\neq q'$, the root $\beta=i+\sum_{p=1}^rq_p$ is imaginary. Denoting the arrow from $i$ to $j$ by $\gamma$, there exists a Schurian representation of dimension $\beta$ with $X_\gamma=0$ which implies $\Hom(S_i,X_\gamma)\neq 0$.
  ...
  is imaginary with $\hom(i,\beta)\neq 0$ and Lemma \ref{lem: non-preinjective} applies.
	...
	It remains to consider the case when $|Q_0(-,i)|=1$ and removing all arrows $\alpha\in Q_1(i,-)$ does not create a disconnected component $Q'$  of type $A$ with leaf $\ell$ where $\gamma:\ell\to i$ denotes the unique arrow with sink $i$. Let $Q'$ be the connected component containing $\ell$ after removing all arrows from $Q_1(i,-)$.
...
Finally, assume that $Q'$ is of Dynkin type $D$ or $E$ (with leaf $i$). Moreover, we can assume that removing $\gamma$ yields precisely two connected components, the one of $\ell$ and the one of $i$. Indeed, otherwise, we can proceed as in the respective case for $Q_0(-,i)\geq 2$. Applying reflection functors we can assume that all arrows in $Q'$ are oriented towards $\ell$, i.e. $\ell$ is a sink of $Q'$. By assumption, we either have $Q_0(-,\ell)=2$ or there is a vertex $\ell'\in Q'0$ such that $Q_0(-,\ell')=2$ and $Q_0(\ell',-)=1$. Moreover, there is a unique path from $\ell'$ to $i$. Now let $Q(\ell',i)$ be the quiver obtained from $Q$ when identifying $\ell'$ and $i$ and removing all arrows and vertices along the path from $\ell'$ to $i$. Denote the vertex obtained via this identification by $(\ell',i)$. It is straightforward to check that roots $\beta$ of $Q$ with $\beta_i=\ldots=\beta_\ell'$ are imaginary if and only if the corresponding root of $Q$ with $\beta_{(\ell',i)}$ is imaginary. Thus if $Q(\ell',i)$ is not of Dynkin type, we can make use of the observations in the case $Q(-,i)=2$. Note that $Q(\ell',i)_1(-,i)\geq 2$.
 Thus assume that $Q(\ell',i)$ is of Dynkin type and $Q$ is not. This is only possible if $Q$ is of type $E$. But then for the imaginary Schur root $\delta$ we have $\delta_i<\delta_\ell$. But this yields $\hom(i,\delta)\neq 0$ and thus the claim.
