...
\item there there exists a vertex $l\in N_i$ such that 
\begin{enumerate}
\item $j,q\in K_{l,i}$; 
...
Note that $j$ and $p$ might be equal. The following is straightforward.\sayT{base cases are missing}

\begin{lemma}\sayT{insert proof}If $(j,q)$ is not a dead end with respect to $i$ the pair $(\xi^m_{i,j},N_{i,j}\backslash \{q\})$ is reduced.
\end{lemma}
\begin{proof}
We have to check that $P_{i,q}^0/P_{i,0}$ is not preinjective. If $K_{i,j,q}$ is not $3$-valent, this follows directly from Proposition \ref{pro: injective quotients}. If $K_{i,j,q}$ is $3$-valent both $q$ and $j$ are no leafs of $K_{i,j,q}$. But then again Proposition \ref{pro: injective quotients} implies that $(\xi^m_{i,j},N_{i,j}\backslash \{q\})$ is reduced.
\end{proof}
