...
\subsection{Notation}
\begin{itemize}
  \item Given two tuples $x=(x_1,\ldots,x_n)$ and $y=(y_1,\ldots,y_m)$, we write $x\frown y$ for their concatenation $(x_1,\ldots,x_n,y_1,\ldots,y_m)$. (better notation?)
...
For $i\in Q_0$ and $m\in\ZZ_{\ge0}$, consider the tuple of representations $\cP_{i,m}=\big(P_{k,m}\big)_{k\in Q_1(-,i)} \frown\big(P_{j,m+1}\big)_{j\in Q_1(i,-)}$ and the tuple of vector spaces $\cI_{i,m}=\big(\Irr(P_{k,m},P_{i,m+1})\big)_{k\in Q_1(-,i)} \frown\big(\Irr(P_{j,m+1},P_{i,m+1}\big)_{j\in Q_1(i,-)}$.
