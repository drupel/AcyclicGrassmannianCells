...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item Given $m\ge0$ and $0\le r\le k$, if $m_r\ge0$, then we have an exact sequence
      ...
      \end{itemize}
    \item Given $m\ge0$, if there exists $0\le r\le k+1$ with $m_{r+1}=0$ and $m_r=-1$, then each sectional morphism $P_{i,m}\to P_{i_s,m^*_s}$ is injective for $k-r\le s\le k$ and in this case there is a short exact sequence
      \[\ses{P_{i,m}}{P_{i_s,m^*_s}}{I_{i_{k-s},m_{k-s}}}.\]
      %there exists a filtration
      %\[P_{i,m} \subset P_{i_k,m^*_k} \subset P_{i_{k-1},m^*_{k-1}} \subset \cdots \subset P_{i_{k-r},m^*_{k-r}}\]
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item Suppose $|Q_1(i,-)\sqcup Q_1(-,i)|\ge4$.
      Then
      \begin{enumerate}
        \item The map $P_{i_k,m_k}\oplus P_{i'_{k'},m'_{k'}}\to P_{i,m}$ is injective.
      \end{enumerate}
    \item there exists an exact sequence 
      \[\ses{P_{i,m}}{P_{i_k,m_k} \oplus P_{i'_{k'},m'_{k'}}}{P_{i,m+1}^{\hat L}}\]
...
\end{proposition}
\begin{proof}
  To see (2), we note that there is a commutative diagram
  \[
    \xymatrix{
      & & 0 \ar[d] & 0 \ar[d] & \\
      & & P_{i'_{r'},m'_{r'}} \ar@{=}[r]\ar[d] & P_{i'_{r'},m'_{r'}} \ar[d] & \\
      0 \ar[r] & P_{i_r,m_r} \ar[r]\ar@{=}[d] & P_{i,m} \ar[r]\ar[d] & P_{i_{k-r},m^*_{k-r}} \ar[r]\ar[d] & 0 \\
      0 \ar[r] & P_{i_r,m_r} \ar[r] & P_{i'_{k'-r'},{m'}^*_{\! k'-r'}} \ar[r]\ar[d] & X \ar[r]\ar[d] & 0 \\
      & & 0 & 0 & 
    }
  \]
\end{proof}
