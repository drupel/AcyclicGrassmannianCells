...
\begin{itemize}
  ...
  \item Given two tuples of vector spaces $\cU=(U_1,\ldots,U_n)$ and $\cW=(W_1,\ldots,W_n)$, we write $\cU\subset\cW$ if $U_i$ is a subspace of $W_i$ for each $i$ and $\cU\subsetneq\cW$ if at least one of these subspaces is proper.
    In this case we write $\codim_\cW(\cU)=\sum\limits_{i=1}^n \dim(W_i/U_i)$.
...
Without loss of generality, we will assume $Q_0=\{1,\ldots,n\}$ is labeled along a \emph{sink adapted sequence}, i.e.~vertex 1 is a sink in $Q$, vertex $2$ is a sink in $\mu_1 Q$, vertex $3$ is a sink in $\mu_2\mu_1 Q$, etc.
...
\begin{itemize}
  ...
  \item write $n_{ij}=|Q_1(i,j)|$ for the number of arrows from vertex $i$ to vertex $j$.
\end{itemize}

For a sink or source vertex $k\in Q_0$, write $\Sigma_k:\rep Q\to\rep \mu_k Q$ for the BGP-reflection functor \cite{BGP??}.
...
In particular, the functor $\Sigma_k^2$ is the identity when restricted to the full subcategory $\rep^{\langle k\rangle} Q\subset\rep Q$ consisting of representations with no summand isomorphic to the vertex simple $S_k$.

For $i\in Q_0$, write $P_i$ for the projective cover of the simple $S_i$.
These representations are constructed inductively via $P_i=\Sigma_1\cdots\Sigma_{i-1}S_i$, where $S_i$ here denotes the vertex simple of the quiver $\mu_{i-1}\cdots\mu_1 Q$.
...
%\begin{lemma}
%  For $i,j\in Q_0$, we have $\dim\Irr(P_j,P_i)=n_{ij}$.
%\end{lemma}
The \emph{preprojective} representations $P_{i,m}$ for $i\in Q_0$ and $m\in\ZZ_{\ge0}$ are defined inductively by
\[P_{i,0}=P_i,\qquad P_{i,m+1}=\tau^{-1} P_{i,m},\]
where $\tau:\rep Q\to\rep Q$ denotes the Auslander-Reiten translation.
The Auslander-Reiten translation can be identified with the product of reflection functors $\Sigma_n\cdots\Sigma_1$ \cite{BB??}.
...
To do this efficiently, we need to introduce the universal covering quiver $\widetilde{Q}$ of $Q$.
\begin{definition}
...
This forgetful functor intertwines the actions of the reflection functor $\Sigma_k:\rep Q\to\rep Q$ for $k$ a sink (resp.~source) vertex and the composite reflection functor $\widetilde{\Sigma}_k:\rep\widetilde{Q}\to\rep\widetilde{Q}$ given by applying all reflections $\Sigma_{(k,w)}$, $w\in W$, associated to sinks (resp.~sources) of $\widetilde{Q}$ covering vertex $k$.
...
\begin{lemma}
  \label{le:hom lifts}
  Given representations $M,N\in\rep Q$ which lift to $\widetilde{Q}$, there is an isomorphism $\Hom_Q(M,N)\cong\bigoplus\limits_{w\in W} \Hom_{\widetilde{Q}}(w.\widetilde{M},\widetilde{N})$.
\end{lemma}
\begin{lemma}
  For $i,j\in Q_0$ and $m\ge0$, each of the spaces 
  ...
  for $\alpha\in Q_1(i,j)$ is one-dimensional.
\end{lemma}
\begin{proof}
...
\end{definition}
\begin{corollary}
  For $i,j\in Q_0$ and $m\ge0$, the morphism spaces $\Irr(P_{j,m},P_{i,j})$ and $\Irr(P_{i,m},P_{j,m+1})$ are both $n_{ij}$-dimensional.
\end{corollary}
\begin{remark}
  The notation $\Irr(P_{j,m},P_{i,j})$ and $\Irr(P_{i,m},P_{j,m+1})$ is justified by the following observation: these spaces map isomorphically to the appropriate spaces $\Rad(M,N)/\Rad^2(M,N)$ of irreducible morphisms.
  For the results that follow, it will be important to have honest morphisms and we fix these particular choices of lifts from this quotient space.
\end{remark}

For $i\in Q_0$ and $m\in\ZZ_{\ge0}$, consider the tuple of representations $\cP_{i,m}=\big(P_{k,m}\big)_{k\in Q_1(-,i)} \smile\big(P_{j,m+1}\big)_{j\in Q_1(i,-)}$ and the tuple of vector spaces $\cI_{i,m}=\big(\Irr(P_{k,m},P_{i,m+1})\big)_{k\in Q_1(-,i)} \smile\big(\Irr(P_{j,m+1},P_{i,m+1}\big)_{j\in Q_1(i,-)}$.

\begin{proposition}
  \label{prop:standard AR sequences}
  ...
  \begin{enumerate}
    ...
    \item if there exists an Auslander-Reiten sequence
      ...
      with $\ell\in Q_1(-,i)$ and $\dim\Irr(P_{\ell,m},P_{i,m+1})=1$, then the sequence \eqref{eq:preprojective AR} induces an exact sequence
      \begin{equation}
        \label{eq:reduced AR 1}
        \ses{P_{\ell,m-1}}{\cP_{i,m}\bullet\cI_{i,m}^{\{\ell\}}}{P_{i,m+1}};
      \end{equation}
      %\begin{equation}
      %  \label{eq:reduced AR 1}
    ...
    \item if there exists an Auslander-Reiten sequence
      ...
      with $\ell\in Q_1(i,-)$ and $\dim\Irr(P_{\ell,m+1},P_{i,m+1})=1$, then the sequence \eqref{eq:preprojective AR} induces an exact sequence
      \begin{equation}
        \label{eq:reduced AR 2}
        \ses{P_{\ell,m}}{\cP_{i,m}\bullet\cI_{i,m}^{\{\ell\}}}{P_{i,m+1}};
      \end{equation}
      %\begin{equation}
      %  \label{eq:reduced AR 2}
...
\end{proposition}
\begin{proof}
  Part (1) can be reduced to the case $m=0$ by applying Auslander-Reiten translations and then to the case where vertex $i$ is a sink in $Q$ using BGP-reflection functors (APR-tilting).
  Thus it suffices to establish the existence of an Auslander-Reiten sequence 
  \[\ses{P_{i,m}}{\bigoplus\limits_{k\in Q_1(-,i)} P_{k,m}\otimes\Irr(P_{k,m},P_{i,m+1})}{P_{i,m+1}}\]
  when $i\in Q_0$ is a sink.

  To see part (2), we consider the commutative diagram:
  \[\xymatrix{
      0 \ar[d] & 0 \ar[d] \\
      P_{\ell,m-1} \ar[d]\ar[r]\ar@{}[dr]|(.7){\lrcorner} & \cP_{i,m}\bullet\cI_{i,m}^{\{\ell\}} \ar[d] \\
      P_{i,m} \ar[d]\ar[r] & \cP_{i,m}\bullet\cI_{i,m} \ar[d] \\
...
\begin{lemma}
  For any $\cV\subsetneq\cI_{i,m}$, the natural map $\cP_{i,m}\bullet\cV\to P_{i,m+1}$ is injective (assuming we are not in cases (2) or (3) of Proposition~\ref{prop:standard AR sequences}, otherwise $\cI_{i,m}$ should be replaced by $\cI_{i,m}^{\{\ell\}}$).
\end{lemma}
\begin{definition}
