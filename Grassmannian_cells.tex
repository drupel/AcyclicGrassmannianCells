...
We mostly deal with the case when $M_1\xrightarrow{f_1} M_2\xrightarrow{f_2}\ldots\xrightarrow{f_{r-1}} M_r$ is a chain of irreducible monomorphisms between exceptional representations $M_i$ which means that $\Hom(M_i,M_j)=0$ for $j>i$ and $\Ext(M_i,M_j)=0$\sayT{double-check if this is always this true? In the preprojective component it is} and $\Hom(M_i,M_{i+1})=\kk$. Then the maps $\phi_{i,j,k}$ are unique up to scalars. Furthermore, we have Auslander-Reiten sequences
$$\ses{M_i}{ \tau^{-1}M_{i-1}\oplus\bigoplus_{j=1}^{l_i}N_{i,j}\oplus M_{i+1}}{\tau^{-1}M_i}$$ with indecomposables $N_{i,j}$. Write $N_i=\bigoplus_{j=1}^{l_i}N_{i,j}$. 
...
Let $0=M_0\subset M_1\subset M_2\subset \ldots \subset M_r$ be a $\tau$-stable chain of representations\sayD{definition?}. For $0\leq i< j< k\leq r$, we have $\tau$-stable short exact sequences
$$\ses{M_j/M_i}{M_k/M_i}{M_k/M_j}$$ and thus Auslander-Reiten theory induces a morphisms $\phi_{i,j,k}:M_j/M_i\to\tau (M_k/M_j)$.
 
\begin{lemma}
  \label{lem:mochain}
  Let $0=M_0\subset M_1\subset M_2\subset \ldots \subset M_r$ be a $\tau$-stable chain of representations such that for each $1\le j<r$ the induced map $\phi_{0,j,j+1}:M_j\to \tau M_{j+1}/M_j$ is surjective and such that $\ker(\phi_{0,j,j+1})=M_{j-1}\oplus K_j$ for some representations $K_j$.
...
\begin{proof}
  ...
  is $\tau$-stable, we end up with an exact sequence
