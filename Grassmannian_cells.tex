...
\sayT{$\ell$ itself should probably be a leaf of this component? Or do we want to include cases where $\ell$ is a middle vertex of a type $A$ component?}
\sayD{Part of the point is that a limb controls a ``wing'' in the AR quiver that looks more or less like an infinite version of the AR quiver for a linearly oriented type A component where all mesh relations are simply squares, this is essentially why Corollary 3.15 holds.}
...
\begin{proof}
  ...
  If $\ell$ is a limb with respect to vertex $n$ and $Q_0(n,-)=\{\ell,\ell_1,\ell_2\}$, then an easy calculation shows that $E\cong\tau^{-r}P_{\ell',0}$ is preprojectivei, here $\ell'$ is the leaf vertex of $Q$ in the subquiver $Q(\ell)\subset Q$ and $r$ is the number of arrows in $Q(\ell)$ directed towards $\ell'$.
