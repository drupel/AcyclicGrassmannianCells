...
\begin{proof}
  ...
  If $Q'$ is wild or of extended Dynkin type, let $M$ be any indecomposable representation of $Q'$ such that $\beta=\udim M$ is an imaginary root of $Q'$ and thus an imaginary root of $Q$.
  ...
  Now $S_i$ is preprojective as a representation of $Q''$.
  \sayD{How do we conclude from here that $S_i$ is not preinjective as a representation of $Q$?}
  ...
  Thus we may additionally assume that there exists precisely one path from each of the vertices $q,q'$ to $j$.
  If there exists vertex $i'\in Q_0$ different from $i$ such that $|Q_0(i,-)\cup Q_0(-,i)|\geq 3$, it is straightforward to check that $Q$ has a subquiver of extended Dynkin type $D$.
  In this subquiver precisely two vertices out of $\{q,q',j\}$ are leaves.
  With this observation, it is straightforward to reduce to the case $\tilde D_4$ treated before.
  ...
  is imaginary with $\hom(i,\beta)\neq 0$ and Lemma \ref{lem: non-preinjective} applies.
	...
	Thus assume that $Q$ is a tree. As it is not of Dynkin type, it contains an extended Dynkin quiver of type $E$ as a full subquiver. But then it is straightforward that $\hom(i,\delta)\neq 0$ for the respective imaginary Schur roots $\delta$, see \cite[Section 4]{CB} for a list. Thus again Lemma \ref{lem: non-preinjective} applies.
        ...
        \sayD{I don't see why this paragraph is necessary.}
...
% in particular $E$ is projective as a representation of $Q'$.
 ...
 % Indeed, this would imply that $E$ is injective as a representation of $Q'$, but as $\ell$ is not a limb vertex, $Q'$ is not of type A and thus $Q'$ does not admit a projective-injective representation.
  ...
  As $\ell$ is not a limb vertex with respect to $n$ and as $\ell$ is a sink of $Q'':=\mu_{i_{r-1}}\ldots\mu_{i_1}Q$, there exist at least two arrows $\alpha:q\to \ell$ and $\alpha':q'\to \ell$ with $q,q'\neq n$ in $Q''$, i.e. we have $Q_1(-,\ell)\geq 2$.
  \sayD{I don't see why this claim should be true.  For example we could have $|Q_1(\ell,-)\cup Q_1(-,\ell)|=2$ with one of the arrows connected to $n$.}
  But now Proposition \ref{pro:simpleregular} shows that $S_\ell$ and thus $E=\Sigma_{i_1}\cdots\Sigma_{i_{r-2}}\Sigma_n S_\ell$ is not  preinjective.	
...
\end{lemma}
\begin{proof}
Let $H\subset Q_0(t,-)$. By Proposition \ref{pro:simpleregular}, the representation $P_{t,H}^0$ is preinjective if $H=Q_0(t,-)$ or if $|H|=|Q_0(t,-)|-1\neq 0$ and, moreover, the unique vertex $h\in Q_0(t,-)\backslash\{H\}$ is a limb vertex.

By Corollary \ref{cor:simpleregular}, the representation $\mathrm{coker}(f_{t,H}^0)$ is preprojective if $H=\emptyset$ or if $|H|=1\neq |Q_0(t,-)| $ and, moreover, the unique vertex $h\in H$ is a limb vertex.



For $T\subsetneq Q_0(-,h)$, we have 
$$\udim P_{h,T}^0=\sum_{\substack{t\in Q_0(-,h)\\t\notin T}}\left(t+\sum_{h'\in Q_0(t,-)}h'\right)-h.$$

Then $P_{h,T}^0$ is an exceptional representation such that
$$\udim\tau\left(P_{h,T}^0\right)=h+\sum_{t\in T}t.$$
Thus by Proposition \ref{pro:simpleregular}, the representation $P_{h,T}^0$ is preinjective if and only if $0\neq |T|=|Q_0(h,-)|-1$ and, moreover, the unique vertex $t\in Q_0(h,-)\backslash T$ is a limb vertex.

By Corollary \ref{cor:simpleregular}, it is preprojective if $|T|=0$ or $|T|=1\neq |Q_0(-,h)|$ and, moreover, the unique vertex $t\in T$ is a limb vertex.
...
\begin{definition}
We call a truncated representation $S$ simple if we have $\Hom(S,X)=0$ for all truncated representations $X\ncong S$.
\end{definition}
