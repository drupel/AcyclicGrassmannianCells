...
\begin{proposition}
  For $i\in Q_0$ and $m\in\ZZ_{\ge0}$, there exists an Auslander-Reiten sequence
  \[\ses{P_{i,m}}{\bigoplus_{k\in Q_1(-,i)} P_{k,m}\otimes\Irr(P_{k,m},P_{i,m+1})\oplus\bigoplus_{j\in Q_1(i,-)} P_{j,m+1}\otimes \Irr(P_{j,m+1},P_{i,m+1})}{P_{i,m+1}}.\] 
\end{proposition}
